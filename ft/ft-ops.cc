...
int toku_ft_layer_init(void) {
    ...
    kibbutz_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "kibbutz_mutex");
    minicron_p_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "minicron_p_mutex");
    queue_result_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "queue_result_mutex");
    tpool_lock_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "tpool_lock_mutex");
    workset_lock_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "workset_lock_mutex");
    bjm_jobs_lock_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "bjm_jobs_lock_mutex");
    log_internal_lock_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "log_internal_lock_mutex");
    ...
    cachetable_disk_nb_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "cachetable_disk_nb_mutex");
    safe_file_size_lock_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "safe_file_size_lock_mutex");
    cachetable_m_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "cachetable_m_mutex_key");
    checkpoint_safe_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "checkpoint_safe_mutex");
    ft_ref_lock_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "ft_ref_lock_mutex");
    ft_open_close_lock_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "ft_open_close_lock_mutex");
    loader_error_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "loader_error_mutex");
    bfs_mutex_key =
        new toku_instr_key(toku_instr_object_type::mutex, "fti", "bfs_mutex");
    loader_bl_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "loader_bl_mutex");
    loader_fi_lock_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "loader_fi_lock_mutex");
    loader_out_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "loader_out_mutex");
    result_output_condition_lock_mutex_key =
        new toku_instr_key(toku_instr_object_type::mutex,
                           "fti",
                           "result_output_condition_lock_mutex");
    block_table_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "block_table_mutex");
    rollback_log_node_cache_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "rollback_log_node_cache_mutex");
    txn_lock_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "txn_lock_mutex");
    txn_state_lock_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "txn_state_lock_mutex");
    txn_child_manager_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "txn_child_manager_mutex");
    txn_manager_lock_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "txn_manager_lock_mutex");
    treenode_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "treenode_mutex");
    locktree_request_info_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "locktree_request_info_mutex");
    manager_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "manager_mutex");
    manager_escalation_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "manager_escalation_mutex");
    db_txn_struct_i_txn_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "db_txn_struct_i_txn_mutex");
    manager_escalator_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "manager_escalator_mutex");
    indexer_i_indexer_lock_mutex_key = new toku_instr_key(
        toku_instr_object_type::mutex, "fti", "indexer_i_indexer_lock_mutex");
    ...
    tokudb_file_data_key = new toku_instr_key(
        toku_instr_object_type::file, "fti", "tokudb_data_file");
    tokudb_file_load_key = new toku_instr_key(
        toku_instr_object_type::file, "fti", "tokudb_load_file");
    tokudb_file_tmp_key = new toku_instr_key(
        toku_instr_object_type::file, "fti", "tokudb_tmp_file");
    tokudb_file_log_key = new toku_instr_key(
        toku_instr_object_type::file, "fti", "tokudb_log_file");

    fti_probe_1_key =
        new toku_instr_key(toku_instr_object_type::mutex, "fti", "fti_probe_1");
    fti_probe_2_key =
        new toku_instr_key(toku_instr_object_type::mutex, "fti", "fti_probe_2");
    fti_probe_3_key =
        new toku_instr_key(toku_instr_object_type::mutex, "fti", "fti_probe_3");
    fti_probe_4_key =
        new toku_instr_key(toku_instr_object_type::mutex, "fti", "fti_probe_4");

    extractor_thread_key = new toku_instr_key(
        toku_instr_object_type::thread, "fti", "extractor_thread");
    fractal_thread_key = new toku_instr_key(
        toku_instr_object_type::thread, "fti", "fractal_thread");
    io_thread_key =
        new toku_instr_key(toku_instr_object_type::thread, "fti", "io_thread");
    eviction_thread_key = new toku_instr_key(
        toku_instr_object_type::thread, "fti", "eviction_thread");
    kibbutz_thread_key = new toku_instr_key(
        toku_instr_object_type::thread, "fti", "kibbutz_thread");
    minicron_thread_key = new toku_instr_key(
        toku_instr_object_type::thread, "fti", "minicron_thread");
    tp_internal_thread_key = new toku_instr_key(
        toku_instr_object_type::thread, "fti", "tp_internal_thread");

    result_state_cond_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "result_state_cond");
    bjm_jobs_wait_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "bjm_jobs_wait");
    cachetable_p_refcount_wait_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "cachetable_p_refcount_wait");
    cachetable_m_flow_control_cond_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "cachetable_m_flow_control_cond");
    cachetable_m_ev_thread_cond_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "cachetable_m_ev_thread_cond");
    bfs_cond_key =
        new toku_instr_key(toku_instr_object_type::cond, "fti", "bfs_cond");
    result_output_condition_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "result_output_condition");
    manager_m_escalator_done_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "manager_m_escalator_done");
    lock_request_m_wait_cond_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "lock_request_m_wait_cond");
    queue_result_cond_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "queue_result_cond");
    ws_worker_wait_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "ws_worker_wait");
    rwlock_wait_read_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "rwlock_wait_read");
    rwlock_wait_write_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "rwlock_wait_write");
    rwlock_cond_key =
        new toku_instr_key(toku_instr_object_type::cond, "fti", "rwlock_cond");
    tp_thread_wait_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "tp_thread_wait");
    tp_pool_wait_free_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "tp_pool_wait_free");
    frwlock_m_wait_read_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "frwlock_m_wait_read");
    kibbutz_k_cond_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "kibbutz_k_cond");
    minicron_p_condvar_key = new toku_instr_key(
        toku_instr_object_type::cond, "fti", "minicron_p_condvar");

    multi_operation_lock_key = new toku_instr_key(
        toku_instr_object_type::rwlock, "fti", "multi_operation_lock");
    ...
    cachetable_m_list_lock_key = new toku_instr_key(
        toku_instr_object_type::rwlock, "fti", "cachetable_m_list_lock");
    ...
    cachetable_m_lock_key = new toku_instr_key(
        toku_instr_object_type::rwlock, "fti", "cachetable_m_lock");
    result_i_open_dbs_rwlock_key = new toku_instr_key(
        toku_instr_object_type::rwlock, "fti", "result_i_open_dbs_rwlock");
    checkpoint_safe_rwlock_key = new toku_instr_key(
        toku_instr_object_type::rwlock, "fti", "checkpoint_safe_rwlock");
    cachetable_value_key = new toku_instr_key(
        toku_instr_object_type::rwlock, "fti", "cachetable_value");
    safe_file_size_lock_rwlock_key = new toku_instr_key(
        toku_instr_object_type::rwlock, "fti", "safe_file_size_lock_rwlock");
    cachetable_disk_nb_rwlock_key = new toku_instr_key(
        toku_instr_object_type::rwlock, "fti", "cachetable_disk_nb_rwlock");
