...
static int toku_recover_frename (struct logtype_frename *l, RECOVER_ENV renv) {
    ...
    std::unique_ptr<char[], decltype(&toku_free)>
        old_iname_full(toku_construct_full_name(2, data_dir, l->old_iname.data),
                &toku_free);
    std::unique_ptr<char[], decltype(&toku_free)>
        new_iname_full(toku_construct_full_name(2, data_dir, l->new_iname.data),
                &toku_free);
    ...
    // new file have not yet created during recovery process the 'stalled cachefile'
    // container can contain only cache file of old file. To preserve the old cachefile
    // file's id and keep it in 'stalled cachefiles' container the new file is removed
...
static int toku_recover_backward_frename (struct logtype_frename *UU(l), RECOVER_ENV UU(renv)) {
    // nothing
    return 0;
}
