...
int
toku_commit_frename (BYTESTRING    /* old_name */,
...
int
toku_rollback_frename (BYTESTRING    old_iname,
                       BYTESTRING    new_iname,
                       TOKUTXN       txn,
                       LSN           UU(oplsn))
{
    ...
    std::unique_ptr<char[], decltype(&toku_free)>
        old_iname_full(toku_cachetable_get_fname_in_cwd(cachetable, old_iname.data),
                &toku_free);
    std::unique_ptr<char[], decltype(&toku_free)>
        new_iname_full(toku_cachetable_get_fname_in_cwd(cachetable, new_iname.data),
                &toku_free);
    ...
    // fs-dependent file id - (device id, inode number) pair. To preserve the new cachefile
    // file's id and keep it in 'stalled cachefiles' container the old file is removed
    ...
    if (r != ENOENT) {
        ...
        //       invokes toku_cachefile_unlink_on_close - lazy unlink on file close,
        ...
        //       when file lock is released the file is closed and unlinked if corresponding flag is set in cachefile object)
        // 5) crash
        //
        ...
        // - create 'a'
        // - rename 'a' to 'b'
        // - unlink 'a' - as 'a' file does not exist we have crash on assert here
        //
        // There is no need to write "unlink" in recovery log in (4a) because 'a' will be removed
