                     ...
                     LSN        UU(oplsn))
{
    ...
    assert(txn->container_db_txn->mgrp);

    toku_struct_stat stat;
    const char *data_dir = txn->container_db_txn->mgrp->get_real_data_dir(txn->container_db_txn->mgrp);
    bool old_exist = true;
    bool new_exist = true;
    ...
    std::unique_ptr<char[], decltype(&toku_free)>
        old_iname_full(toku_construct_full_name(2, data_dir, old_iname.data + 2),
                &toku_free);
    std::unique_ptr<char[], decltype(&toku_free)>
        new_iname_full(toku_construct_full_name(2, data_dir, new_iname.data + 2),
                &toku_free);
    ...
    if (new_exist && old_exist) {
        if (unlink(new_iname_full.get()) == -1)
            return 1;
        return 0;
    }

    if (!old_exist && new_exist) {
        if (rename(new_iname_full.get(), old_iname_full.get()) == -1)
            return 1;
        return 0;
    }
