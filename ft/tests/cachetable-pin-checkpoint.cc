...
cachetable_test (void) {
    ...
    for (int i = 0; i < NUM_MOVER_THREADS; i++) {
        r = toku_pthread_create(
            toku_uninstrumented, &move_tid[i], NULL, move_numbers, NULL);
        assert_zero(r);
    }
    r = toku_pthread_create(
        toku_uninstrumented, &checkpoint_tid, NULL, checkpoints, NULL);
    assert_zero(r);
    r = toku_pthread_create(
        toku_uninstrumented, &time_tid, NULL, test_time, NULL);
    assert_zero(r);
