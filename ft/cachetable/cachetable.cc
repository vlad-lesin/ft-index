...
int evictor::init(long _size_limit, pair_list* _pl, cachefile_list* _cf_list, KIBBUTZ _kibbutz, uint32_t eviction_period) {
    ...
    m_kibbutz = _kibbutz;
    toku_mutex_init(
        *cachetable_ev_thread_lock_mutex_key, &m_ev_thread_lock, NULL);
    toku_cond_init(
        *cachetable_m_flow_control_cond_key, &m_flow_control_cond, NULL);
    ...
    r = toku_pthread_create(
        *eviction_thread_key, &m_ev_thread, NULL, eviction_thread, this);
    if (r == 0) {
