...
static void stress_test(int nelts) {
    ...
    ZERO_STRUCT(extra);
    extra.omt = &omt;
    toku_mutex_init(toku_uninstrumented, &extra.mutex, NULL);
    ...
    for (int i = 0; i < num_marker_threads; ++i) {
        ...
        toku_pthread_create(toku_uninstrumented, &marker_threads[i], NULL, stress_mark_worker, &reader);
    }

    toku_pthread_t deleter_thread;
    toku_pthread_create(toku_uninstrumented, &deleter_thread, NULL, stress_delete_worker, &readers[0]);
