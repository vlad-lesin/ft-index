...
int test_main(int UU(argc), char *const UU(argv[])) {
    ...
    r = toku_pthread_create(toku_uninstrumented, &thread1, NULL, acquire_lock_thread, &e1); CKERR(r);
    acquire_lock_extra e2(txn3, magic_key);
    r = toku_pthread_create(toku_uninstrumented, &thread2, NULL, acquire_lock_thread, &e2); CKERR(r);
